...
/*   Los metodos eran estaticos por lo que no podian acceder a la lista 
 *   La Clase EmpleadoTiempoCompleto no heredaba de la clase Empleado
 *   a los metodos GuardarDatos y CargarDatos les fataba informacion.
 *   faltaban algunos manejos de errores de igual forma.
 *   */
...
namespace Parcial_2
{
    internal class Program
    {
        ...
        {
            Console.WriteLine("SISTEMA PARA EMPLEADOS");
        ...
        void GuardarDatos()
        {
            using (StreamWriter sw = new StreamWriter("empleados.txt"))
            {
                foreach (var empleado in empleados)
                {
                    sw.WriteLine(empleado.GetType().Name);
                    sw.WriteLine(empleado.Nombre);
                    if (empleado is EmpleadoTiempoCompleto empleadoTiempoCompleto)
                    {
                        sw.WriteLine(empleadoTiempoCompleto.Salario);
                    }
                    else if (empleado is EmpleadoPorHoras empleadoPorHoras)
                    {
